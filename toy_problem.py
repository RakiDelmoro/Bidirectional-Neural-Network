...
def generate_input_and_expected_pair(number_will_generated):
    # [[w1, w2], [w3, w4]]
    target_weights = torch.tensor([[3, 5], [7, 9]], dtype=torch.float, device="cuda").inverse()
    bias = torch.tensor([[0.5345, 0.8654]], dtype=torch.float, device="cuda")

    generated_pair_of_number = []
    for _ in range(number_will_generated):
        l2r_pair = torch.tensor([[random.randint(0, sys.maxsize), random.randint(0, sys.maxsize)]], dtype=torch.float, device="cuda")
...
def neural_network_two_to_two(data):
    ...
    * - Neuron
    ...
    l2r_weights = torch.tensor([[0.1233, 0.5365], [0.1457, 0.1371]], dtype=torch.float, device="cuda")
    bias = torch.tensor([[0.1645, 0.5463]], dtype=torch.float, device="cuda")
    ...
    while True:
        ...
        yield f"Loss: {torch.mean(output_nodes_loss)} Left to right output node: {l2r_output_nodes}"
...
def pytorch_neural_network(data):
    weights = torch.tensor([[0.1233, 0.5365], [0.1457, 0.1371]], dtype=torch.float, device="cuda", requires_grad=True)
    bias = torch.tensor([[0.1645, 0.5463]], dtype=torch.float, device="cuda", requires_grad=True)
    ...
    while True:
        ...
        nodes_loss.backward()
        optimizer.step()
        yield f'Loss: {nodes_loss.item()} Neuron Activation: {output_nodes}'
        # yield f'Weights: {weights}'

for _ in range(10):
    data = generate_input_and_expected_pair(1)
    for py_result, custom_result in zip(pytorch_neural_network(data), neural_network_two_to_two(data)):
        print("Result from pytorch")
        print(py_result)
        print("Result from custom model")
        print(custom_result)
